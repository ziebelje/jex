...
jex.type.map_[c]:c.match(jex.type.regexp_)[1].toLowerCase()};jex.type.regexp_=/\s([a-zA-Z]+)/;jex.type.map_={"[object Array]":"array","[object Boolean]":"boolean","[object Date]":"date","[object Function]":"function","[object Number]":"number","[object Null]":"null","[object Object]":"object","[object RegExp]":"regexp","[object String]":"string","[object Undefined]":"undefined"};jex.table=function(a){var b=a.rows||1,c=a.columns||1;this.table_=rocket.createElement("table");this.table_.setAttribute({cellpadding:"0",cellspacing:"0"});this.tbody_=rocket.createElement("tbody");this.trs_=[];this.tds_=[];!0===a.header&&(this.thead_=rocket.createElement("thead"),a=this.render_tr_(c,"th"),this.trs_.push(a),this.thead_.appendChild(a),this.table_.appendChild(this.thead_),b--);rocket.createElement("tbody");for(var d=0;d<b;d++)a=this.render_tr_(c,"td"),this.trs_.push(a),this.tbody_.appendChild(a);
this.table_.appendChild(this.tbody_)};jex.table.prototype.render_tr_=function(a,b){for(var c=rocket.createElement("tr"),d=[],e,f=0;f<a;f++)e=rocket.createElement(b),d.push(e),c.appendChild(e);this.tds_.push(d);return c};jex.table.prototype.fill_row=function(a,b){for(var c=0,d=b.length;c<d;c++)this.td(c,a).innerHTML(b[c])};jex.table.prototype.table=function(){return this.table_};jex.table.prototype.tbody=function(){return this.tbody_};jex.table.prototype.tr=function(a){return this.trs_[a]};
...
function(){c.apply(this,arguments);a.innerHTML("Listeners: "+jex.debug.listener.get_count())}};jex.debug.listener.get_count=function(){var a=0,b,c=rocket.EventTarget.getListenerTree(),d;for(d in c){b=parseInt(d,10);for(var e in c[b])for(var f in c[b][e])a+=c[b][e][f].length}return a};jex.prettyprint=function(a){this.object_=a.object;var b=a.color_scheme||"dark";this.display_line_numbers_=void 0!==a.display_line_numbers?a.display_line_numbers:!0;this.color_scheme_={};"string"===jex.type(b)?rocket.extend(this.color_scheme_,jex.prettyprint.color_schemes_[b]):rocket.extend(this.color_scheme_,b)};jex.prettyprint.indent_amount_=2;jex.prettyprint.default_text_color_="#000000";jex.prettyprint.default_background_color_="#ffffff";
...
jex.hotkey.normalize_=function(a){a=rocket.trim(a).replace(/\s{2,}/g," ").toLowerCase();a=a.replace("ctrl+shift","shift+ctrl");a=a.replace("alt+shift","shift+alt");a=a.replace("alt+ctrl","ctrl+alt");a=a.replace("shift+alt+ctrl","shift+ctrl+alt");a=a.replace("ctrl+shift+alt","shift+ctrl+alt");a=a.replace("ctrl+alt+shift","shift+ctrl+alt");a=a.replace("alt+shift+ctrl","shift+ctrl+alt");a=a.replace("alt+ctrl+shift","shift+ctrl+alt");a=a.replace("plus","+");a=a.replace("return","enter");a=a.replace("ins",
"insert");a=a.replace("del","delete");a=a.replace("pgup","pageup");a=a.replace("pgdn","pagedown");a=a.replace("pgdown","pagedown");a=a.replace("break","pause");return a=a.replace("esc","escape")};
...
rocket.JSON.stringify(this.xhr_.data.batch)}this.xhr_.data.api_key=jex.cora_request.api_key_;this.xhr_.open("POST",jex.cora_request.api_uri_);this.xhr_.setRequestHeader("Content-type","application/x-www-form-urlencoded");this.xhr_.addEventListener("success",function(){a.execution_end_=new Date;try{var b=rocket.JSON.parse(this.responseText)}catch(c){if(jex.console.error("API call failed (could not decode JSON)"),a.error_callback_){a.error_callback_();return}}!1===b.success?(jex.console.error("API call failed (success false)"),
a.error_callback_&&a.error_callback_()):a.success_callback_&&a.success_callback_(b.data)});this.xhr_.addEventListener("error",function(){jex.console.error("API call failed (status not 200)");a.execution_end_=new Date;a.error_callback_&&a.error_callback_()});this.execution_start_=new Date;this.xhr_.send()};jex.cora_request.prototype.abort=function(){this.xhr_.abort()};
...
jex.cora_request.api_call.prototype.get_object=function(){var a={resource:this.resource_,method:this.method_};this.alias_&&(a.alias=this.alias_);this.arguments_&&(a.arguments=rocket.JSON.stringify(this.arguments_));return a};jex.cookie=function(a,b){if(1===arguments.length){for(var c=a+"=",d=document.cookie.split(";"),e=0;e<d.length;e++){for(var f=d[e];" "==f.charAt(0);)f=f.substring(1,f.length);if(0==f.indexOf(c))return decodeURIComponent(f.substring(c.length,f.length))}return null}};jex.version="14.01";
